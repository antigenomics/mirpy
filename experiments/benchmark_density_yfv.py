...
def main(donor: str = "Q1", cap: int = 0) -> None:
    ...
    print(f"\n{'day':<6}{'clones':>9}{'LLW_in':>8}{'hits':>9}{'LLW_hits':>10}{'recall':>8}")
    counts = {}
    for day in ("0", "15"):
        ...
        res = neighbor_enrichment(obs_emb, bg_emb, lambda0=5.0)  # ALICE: Poisson vs P_gen
        ...
        print(f"d{day:<5}{obs.height:>9}{llw_in:>8}{hits.height:>9}{llw_hits:>10}{recall:>7.0%}")

    verdict = "PASS" if counts["15"] > counts["0"] else "FAIL"
    print(f"\n[{verdict}] LLW-matching enriched hits: d15={counts['15']} > d0={counts['0']}")
