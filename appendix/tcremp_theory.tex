...
\end{proposition}
\begin{proof}[Sketch]
Coverage-completeness with cross-reactivity makes each receptor bind $\Theta(\lvert\mathcal A\rvert)$
antigens and each antigen be bound by a $\ge p_0$ fraction of the repertoire, so the antigen-marginal of the
binding indicator is uniformly bounded inside $(0,1)$; its mutual information with $X$, integrated over
$\mathcal A$, is therefore governed by the clone-frequency structure $\Pgen$ imposes rather than by any
low-dimensional antigen axis, and the complete-coverage limit of the supervised objective coincides with the
generation-sufficient representation. \qed
\end{proof}
