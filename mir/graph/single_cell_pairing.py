"""Single-cell clonotype pairing graph construction."""
...
def build_pairing_graph(
    sample: PairedRepertoire,
    *,
    min_shared_cells: int = 1,
) -> PairingGraph:
    """Build clonotype pairing graph from paired-clonotype sample output."""
    if min_shared_cells < 1:
        raise ValueError("min_shared_cells must be >= 1")

    barcode_pairs = set(sample.single_cell_repertoire.barcode_pair_ids)
    pair_cell_counts: dict[str, int] = {}
    for _, pair_id in barcode_pairs:
    ...
    for rep in sample.paired_locus_repertoires.values():
    ...
    for pair_id, records in pair_to_records.items():
        ...
        for left, right in records:
            ...
            edge_weights[record_key] = edge_weights.get(record_key, 0) + weight
