...
vdjtools    VDJtools clonotype tables (tab-delimited, plain or gzipped).
...
VDJdb slim, human only:
    python -m mir.utils.parser_cli vdjdb.slim.txt.gz -o vdjdb.airr.tsv -f vdjdb_slim
"""
...
def _parse_source(parser_name: str, src: Path):
    parser = _PARSERS[parser_name]()
    if parser_name == "vdjtools":
        sample_id = src.name.removesuffix(".gz").removesuffix(".txt").removesuffix(".tsv")
        return SampleRepertoire.from_clonotypes(parser.parse(str(src)), sample_id=sample_id)
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap.add_argument(
        ...
        action="store_true",
        default=False,
        help="Write gzip-compressed output (.airr.tsv.gz). Also inferred when --output ends in .gz.",
    )
    ...
    if len(sources) > 1:
        if output.exists() and not output.is_dir():
            print(
                "Output must be a directory when multiple input files are given.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    for i, src in enumerate(sources, 1):
        ...
        sample = _parse_source(args.fmt, src)
