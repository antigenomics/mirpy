...
def test_ensure_synthetic_control_rebuilds_unreadable_cache(tmp_path: Path, monkeypatch) -> None:
    ...
    assert Path(rec.path).exists()
    assert len(loaded) == 1
    assert float(loaded["log2_pgen"][0]) == -12.5
...
def test_synthetic_controls_with_different_n_have_distinct_manifest_records(tmp_path: Path) -> None:
    ...
    assert int(loaded_small["duplicate_count"][0]) == 1
    assert int(loaded_large["duplicate_count"][0]) == 2
...
def test_ensure_synthetic_control_reuses_larger_cache_by_prefix(tmp_path: Path, monkeypatch) -> None:
    ...
    with open(rec.path, "rb") as fh:
        got = pickle.load(fh)
    assert len(got) == 3
    assert got["duplicate_count"].to_list() == [1, 2, 3]
...
def test_ensure_synthetic_control_extends_smaller_cache(tmp_path: Path, monkeypatch) -> None:
    ...
    with open(rec.path, "rb") as fh:
        got = pickle.load(fh)
    assert len(got) == 4
    assert got["junction"].to_list() == ["A", "B", "C", "D"]
